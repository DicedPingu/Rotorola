...
const List<Item> itemsList = [
  ...
  Item(
    id: 101,
    title: "Disable Moto App Cloud (Aura)",
    description: "Prevent automatic installation of carrier bloatware.",
    guide: "Open Termux (run `su`) or run ADB shell from computer: `pm disable-user --user 0 com.aura.oobe.moto`. This shuts down the background downloader service.",
    category: ItemCategory.doImmediately,
    whatIsIt: "A background carrier-stub app bundled by Motorola that silently downloads partner advertisement apps and games.",
    whyDoIWantIt: "Stops unwanted applications from cluttering your home screen, reduces background data usage, and increases battery life.",
  ),
  Item(
    id: 102,
    title: "Disable Motorola RAM Boost",
    description: "Turn off virtual RAM swap on internal storage.",
    guide: "Go to Settings > System > Performance > RAM Boost, toggle it to OFF, and reboot. Alternatively, run: `settings put global ram_expand_size 0`.",
    category: ItemCategory.doImmediately,
    whatIsIt: "A virtual swap feature that uses the phone's slow eMMC/UFS storage chips to act as extra volatile memory.",
    whyDoIWantIt: "Storage chips are hundreds of times slower than physical RAM. Disabling RAM Boost stops micro-stuttering and extends the life of your internal storage.",
  ),
  Item(
    id: 103,
    title: "Tweak MediaTek Thermal Conf",
    description: "Optimize thermal throttling profiles.",
    guide: "Use a root file manager to locate `/vendor/etc/thermal-map.conf` and adjust peak throttling temperatures, or install the 'MTK Thermal Controller' Magisk module.",
    category: ItemCategory.goodToDo,
    whatIsIt: "A system configuration map that dictates how the MediaTek MT6768 chip reduces its clocks when heating up.",
    whyDoIWantIt: "Allows the budget Helio G85 chip to run at peak frequency longer during gaming sessions, avoiding sudden frame rate drops.",
  ),
  Item(
    id: 104,
    title: "Mount System Read-Write (OverlayFS)",
    description: "Write files directly to read-only partitions.",
    guide: "Install the **Magic OverlayFS** Magisk module. Open Termux, type `su`, and run `overlayfs` to configure directory mirrors, enabling system directory changes.",
    category: ItemCategory.mindBlowing,
    whatIsIt: "A tool that overlays a read-write virtual filesystem onto Android 14's native read-only EROFS partition system.",
    whyDoIWantIt: "Modern Android blocks system files modifications. This tool bypasses the restriction so you can edit hosts, system fonts, and default overlays directly.",
  ),
  Item(
    id: 105,
    title: "Bypass MediaTek BROM lock",
    description: "Unlock the ultimate low-level recovery interface.",
    guide: "Use `mtk-bypass` utility on your PC. Hold Volume buttons while plugging in your G24 to exploit the bootrom exploit, unlocking read/write access to all partitions.",
    category: ItemCategory.mindBlowing,
    whatIsIt: "A hardware-level bypass for the MediaTek BootROM security key signature check.",
    whyDoIWantIt: "Provides a fallback unbrick interface that can flash partitions even if the device bootloader is completely corrupted.",
  ),
  Item(
    id: 106,
    title: "Generate custom Play Integrity fingerprints",
    description: "Extract certified fingerprints to pass attestation.",
    guide: "Run `play-curl` script in Termux to query and generate custom `pif.json` profile keys from old certified devices, replacing banned public keys.",
    category: ItemCategory.doImmediately,
    whatIsIt: "A custom signature configuration file that Play Integrity Fix module uses to spoof device hardware checks.",
    whyDoIWantIt: "Bypasses Google's frequent ban waves on public keys, ensuring banking and payment apps remain working long-term.",
  ),
  Item(
    id: 107,
    title: "Configure Universal GMS Doze",
    description: "Optimize background battery consumption of Play Services.",
    guide: "Install the **Universal GMS Doze** module via Magisk/MMRL and reboot. This forces Google Play Services to sleep during screen-off states.",
    category: ItemCategory.doImmediately,
    whatIsIt: "A system optimization script that modifies package alarm lists, allowing Google Play Services to enter deep sleep mode.",
    whyDoIWantIt: "Google Play Services is often the main source of idle battery drain. This module extends G24 standby battery life by up to 30%.",
  ),
  Item(
    id: 108,
    title: "Install AshReXcue watchdog",
    description: "Prevent bootloops caused by conflicting modules.",
    guide: "Install **AshReXcue** (formerly AshLooper). If a bootloop occurs, its watchdog disables only the faulty module and hosts a local recovery WebUI.",
    category: ItemCategory.doImmediately,
    whatIsIt: "A recovery utility that monitors Android's boot sequence, using intelligent differential analysis to catch boot failures.",
    whyDoIWantIt: "Saves you from factory resets or flashing partition backups if you accidentally install an incompatible or broken module.",
  ),
  Item(
    id: 109,
    title: "Configure Shamiko root hider",
    description: "Hide root access from modern detection engines.",
    guide: "Enable Zygisk, add target apps to Magisk's DenyList, but DO NOT enable 'Enforce DenyList'. Install **Shamiko** module and reboot to hide Magisk paths.",
    category: ItemCategory.doImmediately,
    whatIsIt: "A Zygisk-based root hiding module that dynamically unmounts root file paths and libraries inside blacklisted app process memory.",
    whyDoIWantIt: "It is far more effective than Magisk's default DenyList, allowing you to bypass strict root checkers in enterprise and banking apps.",
  ),
  Item(
    id: 110,
    title: "Freeze update receivers via shell",
    description: "Block system updates from breaking boot configs.",
    guide: "Execute commands in Termux as root: `pm disable-user --user 0 com.motorola.android.otadm` and `pm disable-user --user 0 com.motorola.omadm`.",
    category: ItemCategory.doImmediately,
    whatIsIt: "System receiver hooks that handle OTA update package checking and download scheduling on Motorola devices.",
    whyDoIWantIt: "Completely blocks background update notifications, preventing accidental OTA downloads that could overwrite your bootloader configuration.",
  ),
  Item(
    id: 111,
    title: "Set SELinux Permissive permanently",
    description: "Disable system kernel sandboxing controls.",
    guide: "Avoid editing `/system/etc/init` to set permissive mode (`setenforce 0`). Permissive kernels globally bypass sandboxing and allow malicious apps to access all processes.",
    category: ItemCategory.avoidOrWrong,
    whatIsIt: "SELinux (Security-Enhanced Linux) is a mandatory access control system in the Android kernel that sandboxes system and user applications.",
    whyDoIWantIt: "Some legacy tweaks require a permissive kernel to write files, but permanently disabling it removes all security boundaries from your rooted device.",
  ),
  Item(
    id: 112,
    title: "Disable thermal mitigation binaries",
    description: "Remove background CPU heat regulators.",
    guide: "Do not delete thermal config files under `/vendor/bin/thermal-engine`. MediaTek chipsets lack redundant safety cutoffs, risking hardware damage or battery swelling.",
    category: ItemCategory.avoidOrWrong,
    whatIsIt: "Kernel protection binaries that throttle the processor when internal temperatures exceed safe operating limits.",
    whyDoIWantIt: "Disabling them to prevent gaming lags can cause extreme overheating, physically melting internal chip solder joints.",
  ),
  Item(
    id: 113,
    title: "Freeze components using App Manager",
    description: "Use root package blocker for granular control.",
    guide: "Install App Manager (OSS). Grant it superuser permissions and use the app blocklist panel to freeze Motorola telemetry services safely.",
    category: ItemCategory.goodToDo,
    whatIsIt: "An advanced open-source application manager that provides root-level package management, manifest inspection, and component disabling.",
    whyDoIWantIt: "Unlike simple uninstallers, freezing keeps package signatures intact so the system doesn't crash, while completely preventing the service from running.",
  ),
  Item(
    id: 114,
    title: "Adjust Low-Memory Killer minfree thresholds",
    description: "Fine-tune RAM memory limits.",
    guide: "Write custom minfree limits to `/sys/module/lowmemorykiller/parameters/minfree` via a startup script to optimize application background caching.",
    category: ItemCategory.goodToDo,
    whatIsIt: "The kernel parameters that dictate at what RAM thresholds Android should start closing background applications.",
    whyDoIWantIt: "Allows the G24's 4GB RAM variant to keep social media or browser pages cached longer, reducing annoying app redraws.",
  ),
  Item(
    id: 115,
    title: "Stop system tracing and logd logging",
    description: "Disable system debugger tracing overhead.",
    guide: "Run `setprop persist.logd.logpersistd off` and `stop logd` in Termux as root. This frees up CPU usage and limits flash memory wear.",
    category: ItemCategory.goodToDo,
    whatIsIt: "Logd is Android's background logging system that continuously records crash logs, debugging printouts, and system stats.",
    whyDoIWantIt: "Stops continuous background logging, saving CPU clock cycles and reducing physical flash storage write wear.",
  ),
  Item(
    id: 116,
    title: "Tune scheduler governor margin metrics",
    description: "Tweak Energy Aware Scheduling (EAS) thresholds.",
    guide: "Modify `/sys/devices/system/cpu/cpufreq/policy0/schedutil/rate_limit_us` to adjust core scaling speed margins for smooth scrolling.",
    category: ItemCategory.goodToDo,
    whatIsIt: "EAS gubernatorial parameters that govern how dynamically work task loads are delegated to big vs. LITTLE processor cores.",
    whyDoIWantIt: "Improves responsiveness and scrolling smoothness by forcing the scheduler to delegate screen workloads to large cores faster.",
  ),
  Item(
    id: 117,
    title: "Downscale rendering size via shell",
    description: "Reduce rendering pixels to save CPU/GPU cycles.",
    guide: "Execute `wm size 720x1560` and `wm density 260` in Termux to downscale the G24's display output, speeding up heavy 3D games.",
    category: ItemCategory.goodToDo,
    whatIsIt: "Android's Window Manager (wm) layout configurations that govern pixel dimension rendering grids and interface scaling rules.",
    whyDoIWantIt: "Downscaling reduces rendering pixels by 20%, easing CPU/GPU load on the Helio G85 to run 3D games at higher, more stable frame rates.",
  ),
  Item(
    id: 118,
    title: "Disable FSync parameter",
    description: "Stop filesystem write sync safety.",
    guide: "Avoid turning off FSync via kernel managers. If the phone runs out of battery, has a kernel panic, or bootloops, all system databases will corrupt.",
    category: ItemCategory.destructive,
    whatIsIt: "A filesystem mechanism that forces the OS to write memory buffers directly to physical storage before confirming completion.",
    whyDoIWantIt: "Disabling it improves writing performance slightly but risks corrupting all app files and system databases during an unexpected crash.",
  ),
  Item(
    id: 119,
    title: "Bypass SSL certificate pinning",
    description: "Force target applications to accept proxy certificates.",
    guide: "Enable LSPosed framework, then install the **TrustMeAlready** module. Activate it for targeted social or web applications to monitor local traffic.",
    category: ItemCategory.mindBlowing,
    whatIsIt: "An Xposed hook that modifies application networking classes to trust custom user certificates, bypassing hardcoded SSL checks.",
    whyDoIWantIt: "Allows you to perform packet analysis and decrypt HTTPS traffic on apps that implement strict certificate pinning.",
  ),
  Item(
    id: 120,
    title: "Run Kali NetHunter chroot",
    description: "Inject and audit networks from your phone.",
    guide: "Install the NetHunter app, download the ARM64 Kali chroot package, and use NetHunter to mount and run Kali terminal tools natively.",
    category: ItemCategory.mindBlowing,
    whatIsIt: "A lightweight Debian-based chroot container compiled with network penetration testing and auditing packages.",
    whyDoIWantIt: "Root access allows the chroot to bind mount sockets and interfaces directly, letting you perform network scans from your G24.",
  ),
];
